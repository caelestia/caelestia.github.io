...
>
>  Since we only increased finitely many $g_j$'s, the result still satisfy the locally finite condition. We can directly see that $\sum_jg'_j$ is nowhere vanishing by construction. Therefore $\{g'_j\}$ is a partition of unity extending $\{f_j\vert_{\phi_i(S^{k-1})}\}$.
>
